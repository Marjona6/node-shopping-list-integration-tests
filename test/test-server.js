...
describe('Recipes', function () {
    ...
    // activate the server before we run the tests
    before(function () {
        return runServer();
    });

    // close the server after we're finished running the tests
    after(function () {
        return closeServer();
    });
    ...
    //   2. inspect response object to make sure response object
    // has the right keys.
    it('should list items on GET', function () {
        // return a promise:
        return chai.request(app).get('/recipes').then(function (res) {
            ...
            var expectedKeys = ['id', 'name', 'ingredients'];
            res.body.forEach(function (item) {
                item.should.be.a('object');
                item.should.include.keys(expectedKeys);
            });
    ...
    it('should add an item on POST', function () {
        ...
        return chai.request(app).post('/recipes').send(newItem).then(function (res) {
            ...
            // response should be deep equal to `newItem` from above if we assign
            // `id` to it from `res.body.id`
            // Q: I don't really understand this part
            res.body.should.deep.equal(Object.assign(newItem, {
                id: res.body.id
            }));
        });
    ...
    // test strategy:
    //  1. GET a recipe item so we can get ID of one
    //  to delete.
    //  2. DELETE an item and ensure we get back a status 200
    it('should delete items on DELETE', function () {
        return chai.request(app
            // first have to get so we have an `id` of item
            // to delete
